    ...
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of MONK earned via Staking & Masternodes"));
	...
	ui->labelAmountMNRewards->setText("0 MONK");
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair MONK
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            piv = (pair["piv"] != 0) ? pair["piv"] / COIN : 0;
            mnrewards = (pair["mn"] != 0) ? pair["mn"] / COIN : 0;
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(142,247,121));
	set1->setColor(QColor(252,229,134));
