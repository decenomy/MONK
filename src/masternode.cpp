...
{
    if (nHeight > 700000) return 100000 * COIN;
    if (nHeight > 600000) return  70000 * COIN;
    if (nHeight > 500000) return  50000 * COIN;
    if (nHeight > 400000) return  30000 * COIN;
    if (nHeight > 300000) return  20000 * COIN;
    if (nHeight > 200000) return  12000 * COIN;
    if (nHeight > 100000) return   8000 * COIN;
...
{
    if (nHeight > 60000) return CRewards::GetBlockValue(nHeight) * 65 / 100;
    if (nHeight >  3000) return CRewards::GetBlockValue(nHeight) * 80 / 100;
    return 0;
}
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when 1000 MONK tx got MASTERNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = UINT256_ZERO;
    CTransaction tx2;
    GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
    BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
    if (mi != mapBlockIndex.end() && (*mi).second) {
        CBlockIndex* pMNIndex = (*mi).second;                                   // block for 1000 MONK tx -> 1 confirmation
